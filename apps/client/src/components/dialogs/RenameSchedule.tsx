 ...
 */
export default function RenameScheduleDialog({ open, setOpen }: Props) {
  const isDarkMode = useSettingsStore((store) => store.isDarkMode)
  const originalName = useScheduleStore((store) => store.schedules[store.scheduleIndex]?.scheduleName)
